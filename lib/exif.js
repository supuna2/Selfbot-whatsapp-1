...
exports.modStick = (media, lindow, tod, from) => {
	out = getRandom('.webp')
	try {
		console.log(media)
		spawn('webpmux', ['-set','exif', './src/sticker/data.exif', media, '-o', out])
		.on('exit', () => {
			lindow.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: tod})
			fs.unlinkSync(out)
			fs.unlinkSync(media)
		})
	} catch (e) {
		console.log(e)
		lindow.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: tod })
		fs.unlinkSync(media)
	}
}

exports.modMedia = (media, lindow, tod, from, fps) => {
	out = getRandom('.webp')
	try {
		ff(media)
		.on('error', (e) => {
			console.log(e)
			lindow.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: tod })
			fs.unlinkSync(media)
		})
		.on('end', () => {
			_out = getRandom('.webp')
			...
			.on('exit', () => {
				lindow.sendMessage(from, fs.readFileSync(_out), 'stickerMessage', { quoted: tod })
				fs.unlinkSync(out)
				fs.unlinkSync(_out)
				fs.unlinkSync(media)
			})
